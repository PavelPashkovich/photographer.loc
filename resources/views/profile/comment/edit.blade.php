                    ...
                    <div class="active tab-pane" id="activity">
                        ...
                        <div class="post clearfix">
                            <div class="user-block">
                                ...
                                <span class="description">Your comment to photo <a href="{{ route('main.photo.index', ['slug' => $comment->photo->slug]) }}"><b>{{ $comment->photo->name }}</b></a>
                                    by <a href="{{ route('main.user.show', $comment->user->slug) }}"><i>{{ $comment->photo->user->name }}</i></a> - {{ $comment->created_at }}</span>
                            </div>
                            <!-- /.user-block -->
                            <p>{{ $comment->comment }}</p>

                            <form class="form-horizontal" action="{{ route('profile.comment.update', $comment) }}" method="post">
                                @method('PUT')
                                @csrf
